...
class Encryption {
    private static $skey = "lewis-scally-likes-goats"; // will need to change this

    private static function b64encode($string) {
        $data = base64_encode($string);
        $data = str_replace(array('+','/','='),array('-','_',''),$data);
        return $data;
    }
 
    private static function b64decode($string) {
    ...
    public static function encrypt($value){ 
        ...
        return trim(self::b64encode($crypttext)); 
    }
 
    public static function decrypt($value){
        if(!$value){return false;}
        $crypttext = self::b64decode($value); 
        $iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB);
        $iv = mcrypt_create_iv($iv_size, MCRYPT_RAND);
